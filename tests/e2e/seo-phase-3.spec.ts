...
test.describe("SEO phase 3 editorial hub", () => {
  test("renders the complete SSR knowledge center across desktop engines", async ({
    page,
  }, testInfo) => {
    test.skip(
    ...
    await expect(page.getByRole("heading", { level: 1 })).toHaveText(
      /Ghiduri pentru fiecare decizie/,
    );
  ...
  test("mobile navigation exposes the hub and restores the page without overflow", async ({
    page,
  }, testInfo) => {
  ...
  test("homepage selection and related-guide navigation are useful", async ({ page }, testInfo) => {
    test.skip(
      ...
      "Product discovery behavior is browser-independent.",
    );
    await page.goto("/", { waitUntil: "domcontentloaded" });
    expect(await page.locator("[data-home-guide-slug]").count()).toBeGreaterThanOrEqual(12);
    await expect(
      page.getByRole("link", { name: /Vezi toate ghidurile despre panouri fotovoltaice/ }),
    ).toBeVisible();

    await page.goto("/calculator-panouri-fotovoltaice", { waitUntil: "domcontentloaded" });
    const related = page
      .getByRole("heading", { name: "Următorul ghid util" })
      .locator("..")
      .getByRole("link");
