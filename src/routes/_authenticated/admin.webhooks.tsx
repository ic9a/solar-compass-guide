...
function Page() {
  ...
  if (!rows) return <AdminLoading label="Se încarcă evenimentele…" />;
  return (
    <AdminPage
      title="Evenimente Stripe"
      description="Urmărește recepționarea și procesarea tehnică a notificărilor."
    >
      {rows.length === 0 ? (
        <AdminEmpty label="Nu există evenimente" />
      ) : (
        <Card className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-left">
              <tr>
                <th className="p-3">Primit</th>
              ...
              {rows.map((e) => (
                <tr key={e.id} className="border-t">
                  <td className="p-3 text-muted-foreground">
                    {new Date(e.received_at).toLocaleString("ro-RO")}
                  </td>
                  <td className="p-3">{e.event_type}</td>
                  <td className="p-3">{e.livemode ? "LIVE" : "test"}</td>
                  <td className="p-3">
                    <AdminStatus value={e.status} />
                  </td>
                  <td className="p-3 font-mono text-[11px] truncate max-w-[200px]">
                    {e.stripe_event_id}
                  </td>
