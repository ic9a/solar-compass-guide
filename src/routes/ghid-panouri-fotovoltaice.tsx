...
function trackGuide(page: EditorialPage, placement: "featured" | "beginner_path" | "category_directory" | "homepage" | "related_guides") {
      ...
      <article>
        <header id="hub-intro" className="relative overflow-hidden bg-[#f4f7f1]">
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-14 sm:px-6 md:py-24 lg:px-8">
            <nav aria-label="Fir de navigare" className="flex items-center gap-1 text-sm text-muted-foreground">
              <Link to="/">Acasă</Link>
            ...
            <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-end">
              <div>
                <p className="product-kicker"><BookOpen className="h-3.5 w-3.5" /> Biblioteca RaportSolar</p>
                ...
                <p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">
                  Înțelegi dimensionarea, producția, costurile, bateria, echipamentele și oferta instalatorului.
                  Poți urma traseul recomandat sau poți merge direct la întrebarea care te preocupă acum.
                  Estimările depind întotdeauna de consum, acoperiș, localitate și configurația reală.
                </p>
              </div>
              <div className="rounded-[1.75rem] border border-brand-green/20 bg-white/85 p-6 shadow-soft">
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  ...
                  Ghidurile explică ipotezele și trimit la surse oficiale acolo unde informația se poate schimba.
                </p>
              </div>
            </div>
          </div>
...
function BeginnerPath() {
  return (
    <section aria-labelledby="beginner-title" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 md:py-24 lg:px-8">
      <div className="max-w-3xl">
        <p className="product-kicker"><Compass className="h-3.5 w-3.5" /> Traseu recomandat</p>
        ...
        <p className="mt-4 leading-7 text-muted-foreground">
          Cele șase etape merg de la necesarul locuinței la verificarea ofertei. Fiecare pas poate fi parcurs separat.
        </p>
      </div>
      <ol className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {BEGINNER_PATH.map((slug, index) => {
          ...
          return (
            <li key={slug} data-beginner-guide={slug} className="relative rounded-[1.6rem] border border-border bg-white p-6">
              <div className="flex items-center justify-between gap-4">
                <span className="grid h-9 w-9 place-items-center rounded-full bg-[#102a2b] text-sm font-bold text-white">
                  {index + 1}
                </span>
                <span className="text-xs font-semibold text-muted-foreground">{page.readingMinutes} min</span>
              </div>
              ...
              >
                Continuă cu pasul {index + 1}<ArrowRight className="h-4 w-4" aria-hidden="true" />
              </Link>
            </li>
          );
        })}
...
function FeaturedGuides() {
  return (
    <section aria-labelledby="featured-title" className="bg-[#102a2b] px-4 py-16 text-white sm:px-6 md:py-24 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <p className="text-sm font-bold uppercase tracking-[0.16em] text-brand-sun">Decizii esențiale</p>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <ArrowRight className="h-5 w-5 text-white/50 transition-transform group-hover:translate-x-1" aria-hidden="true" />
                </div>
                ...
                <p className="mt-6 text-xs font-semibold text-white/50">{page.readingMinutes} minute · {page.audience}</p>
              </Link>
            );
          })}
        </div>
...
function GuideDirectory() {
  return (
    <section aria-labelledby="directory-title" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 md:py-24 lg:px-8">
      <div className="max-w-3xl">
        <p className="product-kicker"><BookOpen className="h-3.5 w-3.5" /> Toate ghidurile</p>
        <h2 id="directory-title" className="mt-5 text-3xl font-bold tracking-[-0.045em] md:text-5xl">
          ...
          return (
            <section key={category.id} aria-labelledby={`category-${category.id}`}>
              <div className="grid gap-3 md:grid-cols-[16rem_minmax(0,1fr)] md:items-end">
                <h3 id={`category-${category.id}`} className="text-2xl font-bold">{category.label}</h3>
              ...
              <ul className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {pages.map((page) => (
                  <li key={page.slug} data-guide-directory-slug={page.slug} className="min-w-0">
                    {page.slug === HUB_SLUG ? (
                      <a href="#hub-intro" className="group flex h-full min-h-52 flex-col rounded-[1.5rem] border border-brand-green/25 bg-brand-green-soft/25 p-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
...
const toolIcons = { recommendation: Calculator, solar_map: MapPinned, offer_analysis: FileSearch } as const;

function ToolActions() {
  const tools: EditorialTool[] = ["recommendation", "solar_map", "offer_analysis"];
  return (
    <section aria-labelledby="tools-title" className="bg-[#f4f7f1] px-4 py-16 sm:px-6 md:py-24 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {tools.map((tool) => {
            ...
            return (
              <Link key={tool} to={item.destination} className="group rounded-[1.7rem] bg-white p-7 shadow-soft">
                <Icon className="h-7 w-7 text-primary" aria-hidden="true" />
                <h3 className="mt-8 text-xl font-bold">{item.label}</h3>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">{item.description}</p>
                <span className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-primary">
                  Deschide instrumentul<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden="true" />
                </span>
              </Link>
            );
          })}
...
function TrustMethod() {
  return (
    <section aria-labelledby="method-title" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 md:py-20 lg:px-8">
      <div className="rounded-[2rem] border border-border bg-white p-7 md:p-10">
        <h2 id="method-title" className="text-2xl font-bold">Cum folosim sursele și estimările</h2>
        <p className="mt-4 max-w-4xl leading-7 text-muted-foreground">
          Datele solare sunt raportate la surse precum JRC/PVGIS, iar procedurile administrative trimit la instituțiile competente.
          Nicio simulare nu înlocuiește măsurarea acoperișului, proiectarea electrică sau verificarea contractului.
          Fiecare ghid indică sursele relevante și explică unde rezultatul rămâne dependent de situația reală.
        </p>
      </div>
    </section>
  );
