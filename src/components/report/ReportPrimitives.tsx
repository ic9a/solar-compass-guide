...
export function ReportShell({ children }: { children: ReactNode }) {
  return (
    <div className="report-shell space-y-5" data-report-shell>
      {children}
    </div>
  );
}
...
export function ReportHeader({
  ...
  return (
    <header className="report-header overflow-hidden rounded-2xl border border-border/70 bg-gradient-hero shadow-soft">
...
export function ReportMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="report-metric rounded-2xl border border-border/70 bg-card p-5 shadow-soft">
...
export function ReportSectionCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="rounded-2xl border border-border/70 bg-card p-5 shadow-soft md:p-6">
      <h2 className="text-lg font-bold">{title}</h2>
      <div className="mt-4">{children}</div>
    </section>
  );
}
export function ReportStrengthsRisks({
  strengths,
  risks,
}: {
  strengths: string[];
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      <ReportSectionCard title="Puncte bune">
        {strengths.length ? (
      ...
      <ReportSectionCard title="De clarificat">
        {risks.length ? (
          <ul className="space-y-2 text-sm">
            {risks.map((x) => (
              <li key={x} className="flex gap-2">
...
export function ReportConfidence({
  ...
  return (
    <ReportSectionCard title="Încrederea în date">
      <p className="text-sm">
        Încredere {label}. Verifică în ofertă orice câmp neclar înainte de semnare.
      </p>
...
export function EmptyState({
  text = "Informația nu este disponibilă în datele extrase.",
}: {
  text?: string;
}) {
        ...
        >
          {tabs.map((t) => (
            <TabsTrigger
              ...
              value={t.id}
              className="min-h-10 whitespace-nowrap px-3 py-2 data-[state=active]:bg-white data-[state=active]:text-brand-green"
            >
              {t.label}
            </TabsTrigger>
          ))}
